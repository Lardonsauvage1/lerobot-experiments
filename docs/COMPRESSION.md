...
**Réponse : non.** Quelle que soit la configuration, le mini-CNN **plafonne entre 45 et 65 % de succès**, et son comportement est **erratique** : sur la ligne du U-Net le plus large (`[128,256,512]`), le succès passe de 62.6 % à 81.2 %, retombe à 32.8 %, remonte à 51.0 %, puis s'écroule à 16.2 % au fil des tailles de jeu de données — sans aucune logique. Le pic à 81.2 % (obtenu avec 100 démos) est un **coup de chance d'entraînement**, pas une vraie performance : son intervalle de confiance ne recouvre pas celui des cellules voisines. Et **agrandir le U-Net n'y change rien** : si l'encodeur visuel ne sait pas lire l'image, le débruiteur en aval n'a aucune information exploitable.
...
*(Dans ce qui suit : le **petit U-Net** = `[64,128,256]`, le **U-Net moyen** = `[128,256,512]`, le **gros U-Net** = `[256,512,1024]`.)*

1. **La vision pure résout Lift (~99 %).** Avec l'architecture compressée de la Partie 1 (le petit U-Net), on plafonnait à ~81 %. Ce n'était pas une limite de la vision pure, mais un **manque de capacité** : en passant au U-Net moyen — à peine deux fois plus de paramètres — le succès à pleines données monte de **81.2 % à 99.4 %**. Autrement dit, l'objectif transférable (résoudre la tâche **sans** coordonnées d'objet) est bel et bien **atteignable**.
2. **Le bon compromis est le U-Net moyen (28.6 M).** Il atteint **99.4 % avec 150 démonstrations** et **99.8 % avec 50**. Doubler encore la taille (le gros U-Net, 76 M) **n'apporte rien** — il fait même un peu moins bien (98.8 %) et devient instable. On a donc **franchi le plateau de capacité** : au-delà du U-Net moyen, plus gros ne veut plus dire mieux.
3. **Ce que coûte le retrait de la béquille.** Le meilleur modèle vision pure (ResNet18 + U-Net moyen, **28.6 M**) pèse environ **17 fois** l'optimum « avec béquille » de la Partie 1 (1.65 M). Ce facteur a **deux causes cumulées** : il faut à la fois un **encodeur visuel plus puissant** (mini-CNN → ResNet18) **et** un **U-Net plus large** (`[32,64,128]` → `[128,256,512]`). Sans la béquille, les deux deviennent nécessaires en même temps.
4. **L'entraînement est plus instable qu'avec la béquille.** Les résultats ne suivent plus une courbe régulière : le gros U-Net donne 83.4 % avec 50 démos mais 94.6 % avec seulement 20 ; le U-Net moyen, lui, retombe à 92.6 % avec 100 démos alors qu'il dépasse 99 % à 50 comme à 150. Sans l'ancrage que fournissaient les coordonnées d'objet, **chaque entraînement part dans une direction plus dispersée**. En pratique, à faible nombre de démos, il faut **entraîner plusieurs fois** (plusieurs seeds) pour conclure de façon fiable.

→ **Bilan transférabilité.** Sur Lift, le bon choix en vision pure est **ResNet18 + U-Net moyen `[128,256,512]` (~29 M)**, pour **~99 % de succès**. C'est toujours **9 fois plus léger** que la baseline de départ (263.7 M), mais loin du facteur 160 annoncé en Partie 1 — qui ne tenait que grâce à la béquille. Le vrai coût d'une compression **transférable** est donc **modéré, pas extrême**.
