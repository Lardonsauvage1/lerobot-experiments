...
| 5 | [`METHODOLOGIE.md`](METHODOLOGIE.md) | Méthodologie (5) | **Peut-on faire confiance à nos mesures ?** 500 rollouts (pas 50) ; le succès **ne se lisse à aucune échelle** (interpoler est invalide) ; **loss au plancher ≠ convergé** (mini-CNN 2 %→74 %) ; aucun proxy offline ne remplace le rollout (`grad_norm` = *quand*, pas *combien*). Protocole expérimental détaillé en annexe. |
...
| [`ENSEMBLING_JOINT.md`](ENSEMBLING_JOINT.md) | **Joint vs cartésien, temporal ensembling & SWA.** L'espace **joint** (moteur) est instable (~15 %, oscille 12-56 %) vs **cartésien** (OSC, 94,8 %). Le **temporal ensembling** le rattrape ponctuellement (30k 56→90 %) mais c'est un *multiplicateur* dépendant d'un checkpoint chanceux. Le **SWA** (moyenne de ~10 checkpoints tardifs) donne **88 % en un seul modèle stable et gratuit** — moyenner les *poids* dépasse le meilleur checkpoint (56 %). Causes de l'instabilité (redondance 7-DOF multimodale, pas d'EMA dans LeRobot). |
