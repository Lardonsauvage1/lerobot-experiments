...
| [`ENSEMBLING_JOINT.md`](ENSEMBLING_JOINT.md) | **Joint vs cartésien, temporal ensembling & stabilisation.** L'espace **joint** (moteur) est instable (~15 %, oscille 12-56 %) vs **cartésien** (OSC, 94,8 %). Le **temporal ensembling** le rattrape ponctuellement (30k 56→**90 %**) mais c'est un *multiplicateur* (∝ marge×instabilité, KO en multimodal), pas une solution stable. Causes (redondance 7-DOF multimodale, pas d'EMA dans LeRobot) + remèdes (EMA, SWA, delta-actions). |
