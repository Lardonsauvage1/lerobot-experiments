...
SRC=results/runs/can/$RUN/checkpoints/040000/pretrained_model
...
echo "[cd $RUN $(date '+%H:%M')] cooldown 5k"
EMA=1 COOLDOWN_STEPS=5000 COOLDOWN_LR0=1e-4 python -u experiments/lift/50_train_valloss.py \
  ...
  --output_dir=results/runs/can/$RUN/cooldown --steps=5000 --save_freq=1000 --log_freq=50 --eval_freq=0 2>&1 | tee /tmp/cd_$RUN.log
echo "[cd $RUN $(date '+%H:%M')] DONE"
