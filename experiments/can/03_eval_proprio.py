"""Phase 5 (Can) — éval du modèle vision-only (image + proprio 9D, SANS coords canette).

Pendant du `02_eval_baseline.py` (qui donnait la can_pos en 12D) mais en vision pure.
val-loss/checkpoint -> best -> rollout 50 val Can @ STEPS pas (max_steps=300, can_eval).
Sortie : results/runs/can/02_proprio_eval.json
"""
import sys
sys.path.insert(0, ".")
...
def state_proprio(obs):
    """9D : proprio seule (eef_pos+eef_quat+gripper). Identique Lift/Can côté env live."""
...
def main():
    ...
    per, agg = lift_eval.rollout_eval_chunked(
        policy, pre, post, init_states, device=device, make_env_fn=can_eval.make_env,
        num_inference_steps=STEPS, stop_on_success=True, max_steps=can_eval.MAX_STEPS,
        fix_obs_fn=lambda o: o, state_fn=state_proprio)
    k = sum(e["success"] for e in per)
    lo, hi = wilson_ci(k, len(per))
    print(f"\n=== CAN image+proprio (9D, SANS canette) : {k}/{len(per)} = {agg['success_rate']:.1%} "
