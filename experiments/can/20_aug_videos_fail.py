"""Vidéos d'ÉCHECS du modèle birdview AUGMENTÉ (18_proprio_birdview_aug, 30 %).

Pendant de 19 mais pour le 18 (crop + color jitter + affine activés au train).
~70 % de fail -> 6 fails attendus dans les 8-12 premières val.
Vidéos en 2 vues côte à côte (agentview | birdview en 256px).

Sortie : results/runs/can/18_proprio_birdview_aug/videos/aug_FAIL_ep<idx>.mp4
"""
import sys
sys.path.insert(0, ".")
...
CKPT = "results/runs/can/18_proprio_birdview_aug/checkpoints/010000/pretrained_model"
OUT = Path("results/runs/can/18_proprio_birdview_aug/videos")
N_FAILS = 6
MAX_TRIES = 15
HD = 256
POLICY_RES = 96
...
def main():
    ...
    for ep in range(len(init_states)):
        ...
        if not success:
            out = OUT / f"aug_FAIL_ep{val_idx[ep]}.mp4"
