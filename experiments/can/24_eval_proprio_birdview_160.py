"""Éval du modèle birdview à 160 px (23_proprio_birdview_160).

Identique à 17_eval_proprio_birdview.py mais avec IMAGE_SIZE=160 (au lieu de 96).
Le script render 160x160 et feed direct au policy (pas de crop côté policy).
"""
import sys
sys.path.insert(0, ".")
...
DATASET_REPO = "local/can_ph_proprio_birdview_160"
DATASET_ROOT = "data_cache/lerobot_can_ph_proprio_birdview_160"
STEPS = 10
...
def main():
    ...
    ap.add_argument("--run-dir", default="results/runs/can/23_proprio_birdview_160")
    ap.add_argument("--out", default="results/runs/can/23_proprio_birdview_160_eval.json")
    args = ap.parse_args()
    ...
    print(f"\n=== CAN 2-cams 160px (agentview+birdview, 9D) : {k}/{n} = {k/n:.1%} "
