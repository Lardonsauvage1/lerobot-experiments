"""Vidéos d'ÉCHECS du modèle 2 cams encodeurs SÉPARÉS (08_proprio_wrist_sep, 55 %).

Iter les val épisodes et garde les N_FAILS premiers échecs.
Vidéos en 2 vues côte à côte (agentview | wrist en 256px → 512×256/frame).

Sortie : results/runs/can/08_proprio_wrist_sep/videos/proprio_FAIL_ep<idx>.mp4
"""
import sys
sys.path.insert(0, ".")
...
CKPT = "results/runs/can/08_proprio_wrist_sep/checkpoints/010000/pretrained_model"
OUT = Path("results/runs/can/08_proprio_wrist_sep/videos")
N_FAILS = 6
MAX_TRIES = 25      # ~45 % de fail -> ~6 fails dans les 13-15 premières val
HD = 256
POLICY_RES = 96
STEPS = 10
MAX_STEPS = 300
...
def main():
    ...
    for ep in range(len(init_states)):
        ...
        for _ in range(MAX_STEPS):
            ...
            frames.append(np.concatenate([ag_hd, wr_hd], axis=1))
