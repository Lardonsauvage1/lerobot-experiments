"""Phase 5 (Can) — éval du modèle 2 cams agentview + BIRDVIEW (16_proprio_birdview).

Pendant de 07 (wrist) mais avec birdview comme 2e caméra. Birdview = vue de dessus
scene-fixée -> parallaxe vraie sans le problème OOD de la wrist.

Sortie : results/runs/can/16_proprio_birdview_eval.json
"""
import sys
sys.path.insert(0, ".")
...
DATASET_REPO, DATASET_ROOT = "local/can_ph_proprio_birdview", "data_cache/lerobot_can_ph_proprio_birdview"
STEPS = 10
...
def render_pair(env, hd=IMAGE_SIZE):
    """Rend agentview + birdview à `hd` px et renvoie (agent, birdview) en torch (3,H,W)."""
    ag = env.env.sim.render(height=hd, width=hd, camera_name="agentview")[::-1]
    bv = env.env.sim.render(height=hd, width=hd, camera_name="birdview")[::-1]
    return (torch.from_numpy(ag.copy()).permute(2, 0, 1).float() / 255.0,
            torch.from_numpy(bv.copy()).permute(2, 0, 1).float() / 255.0)
...
def main():
    ...
    ap.add_argument("--run-dir", default="results/runs/can/16_proprio_birdview")
    ap.add_argument("--out", default="results/runs/can/16_proprio_birdview_eval.json")
    args = ap.parse_args()
    RUN_DIR = Path(args.run_dir); OUT = Path(args.out)
    ...
    print(f"\n=== CAN 2-cams (agentview+birdview, 9D) : {k}/{n} = {k/n:.1%} "
