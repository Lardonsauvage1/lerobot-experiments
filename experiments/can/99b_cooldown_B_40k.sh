...
OUT=results/runs/can/joint_pretrained_cooldown_40k

echo "[99b] $(date '+%H:%M') rapatriement ckpt 24k de B depuis mac2"
mkdir -p "$(dirname $SRC)"
[ -s "$SRC/model.safetensors" ] || ssh -o BatchMode=yes mac2 "cd ~/lerobot-experiments && tar czf - $SRC" | tar xzf - -C . 2>/dev/null
[ -s "$SRC/model.safetensors" ] || { echo "[99b] ckpt 24k absent, abandon"; exit 1; }
...
echo "[99b] $(date '+%H:%M') COOLDOWN par groupe (encodeur 1e-5 / U-Net 1e-4 -> 0 sur 5k) depuis B@40k"
...
echo "[99b] $(date '+%H:%M') éval du cooldown B (LR=0, n=50)"
if [ -s "$OUT/checkpoints/005000/pretrained_model/model.safetensors" ]; then
  ...
  echo "[99b] $(date '+%H:%M') ===== COOLDOWN B : $(tail -1 "$OUT/r.csv"|cut -d, -f4) (vs brut 18% ; from-scratch cooldown 83%) ====="
fi
rm -rf "$OUT/checkpoints" results/runs/can/joint_r34_pretrained/checkpoints/040000