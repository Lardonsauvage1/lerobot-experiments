#!/usr/bin/env bash
# BASELINE avec CANARI — config du run 02, sur le dataset occlus 3 cm.
#
...
LOG=results/logs/can/128_baseline.log
OUT=results/runs/can/b3_baseline
...
rm -rf "$OUT"
# NI CONST_LR NI EMA : on reprend le scheduler par défaut de LeRobot, comme le run 02.
...
for CK in 002500 005000; do
  ...
  $PY -u experiments/can/120_eval_occluded.py --ckpt "$OUT/checkpoints/$CK/pretrained_model" \
      --radius 0.0 --n 30 --tag canari_$CK >> $LOG 2>&1
  LIFT=$($PY -c "
import json
try: print(json.load(open('results/runs/can/occluded/eval_canari_$CK.json'))['can_lifted_frac'])
...
$PY -u experiments/can/120_eval_occluded.py --ckpt $OUT/cooldown/checkpoints/005000/pretrained_model \
    --radius 0.03 --n 150 --tag b3_baseline >> $LOG 2>&1
