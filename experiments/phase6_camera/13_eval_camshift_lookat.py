"""Éval camshift avec perturbation LOOK-AT (cohérente avec l'entraînement 10_make_camaug_lookat).

Variante de 09_eval_camshift_diffusion.py : au lieu d'une rotation d'axe aléatoire (roll parasite),
la caméra est DÉPLACÉE (dérive) puis RÉ-ORIENTÉE vers le plan de travail (look-at) + pan/tilt sans roll.
Sweep de niveaux (trans_cm = rot_deg) ; à chaque niveau L : dérive uniforme 0->L cm + pan/tilt 0->L°.
Sert à comparer PROPREMENT run 31 (caméra fixe) et le futur modèle look-at SOUS LA MÊME perturbation.

Usage :
  venv312/bin/python -u experiments/phase6_camera/13_eval_camshift_lookat.py \
    --ckpt <pretrained_model> --levels 0:0,2:2,5:5,10:10,15:15,20:20 --n 50 --out <csv>
"""
...
def lookat_quat(eye, tgt):
    z = eye - tgt; z = z/np.linalg.norm(z)
...
def cam_setup(env):
    ...
    for c, i in ids.items():
        p0, q0 = orig[c]; f = q2m(q0) @ np.array([0, 0, -1.])
        t = (Z_TABLE - p0[2]) / f[2]; tgt[c] = p0 + f * t
    return ids, orig, tgt


def perturb(env, ids, orig, tgt, trans_m, rot_rad, rng):
    """LOOK-AT : dérive position + ré-orientation vers la cible + pan/tilt (sans roll)."""
    m = env.env.sim.model
    for cam, i in ids.items():
        ...
        if trans_m > 0:
            ...
            d = np.zeros(3)
        new_pos = p0 + d
        base = lookat_quat(new_pos, tgt[cam])
        if rot_rad > 0:
            R = q2m(base); xax, yax = R[:, 0], R[:, 1]
            phi = rng.uniform(0, 2*math.pi); axis = math.cos(phi)*xax + math.sin(phi)*yax
            base = qmul(aa_quat(axis, rng.uniform(0, rot_rad)), base)
        m.cam_pos[i] = new_pos; m.cam_quat[i] = base
    env.env.sim.forward()
...
def eval_level(ckpt, states, device, trans_m, rot_rad, base_seed, max_steps):
    ...
    for i in range(0, len(states), CHUNK):
        env = can_eval.make_env(); ids, orig, tgt = cam_setup(env)
        for ep in range(i, min(i + CHUNK, len(states))):
            obs = env.reset_to({"states": states[ep]}); policy.reset()
            perturb(env, ids, orig, tgt, trans_m, rot_rad, np.random.default_rng(base_seed + ep))
            ok = False
...
def main():
    ...
    print(f"[13] LOOK-AT perturb ckpt={a.ckpt} device={device} n={a.n} levels={levels}", flush=True)
    wh = not out.exists()
    for trans_cm, rot_deg in levels:
        if (trans_cm, rot_deg) in done:
            print(f"[13] {trans_cm}cm/{rot_deg}deg déjà fait, skip", flush=True); continue
        ...
        print(f"[13] {trans_cm}cm/{rot_deg}deg : {res['n_success']}/{res['n']} = {res['success_rate']:.1%} "
