"""
Expérience 42 — Lift : ResNet18 DÉGELÉ (fine-tuné) + anti-overfit combo.

Différence majeure vs run 40 :
- ResNet18 inclus dans le modèle et trainable (sauf BatchNorm gelées)
- 2 LR groups : backbone lr=1e-5 (très bas), head lr=1e-3 (comme avant)
- AdamW global avec weight_decay=1e-4 sur backbone, 5e-4 sur head

C'est la recette LeRobot officielle pour ACT/Diffusion : ResNet pré-entraîné
ImageNet mais fine-tuné lentement sur la tâche. On gardait le ResNet gelé
dans tous les runs précédents (33-41), ce qui empêchait toute adaptation
visuelle au cube/scène spécifique.

Hypothèse : c'est LA cause structurelle qui plafonne à 50%.

Reste identique à run 40 : Dropout 0.4, LayerNorm, label smoothing 0.1,
CE+résidu sur 21 bins × 7 dims × chunk 20, image 96×96 agentview.
"""
...
# ============================================================
EXPERIMENT_NAME = "lift/42_lift_unfrozen_resnet"
...
class ImageStateE2E(nn.Module):
    ...
    def __init__(self, state_dim, hidden_dims, chunk_size, action_dim, n_bins,
        ...
        # ResNet18 (pré-entraîné ImageNet, BN trainable — pas de freeze pour stabilité MPS)
...
def train_model(model, images, img_idx, XS, Y, train_idx, test_idx):
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0, len(train_idx), BATCH_SIZE):
            ...
            # .contiguous() crucial sur MPS : fancy indexing produit non-contigu
...
def main():
    ...
    (train_losses, test_losses, ce_train, mse_train, epoch_times, train_time,
     ...
     best_state_dict, best_success, best_epoch) = train_model(
        model, images, img_idx, XS, Y, train_idx, test_idx
    )
