"""
Expérience 44 — Lift : Run 42 reproductible et robuste (BN trainable préservé).

Run 43 (FrozenBatchNorm) a éliminé le NaN mais aussi le breakthrough à 66% :
les 66% de run 42 venaient justement de la BN qui apprenait les stats Lift.
Geler complètement la BN tombe à 30%.

Run 44 = run 42 (BN trainable, donc 66% atteignable) + 2 fixes de run 43 :
1. NaN guard : skip batches avec loss NaN/inf
2. Save best checkpoint à disque IMMÉDIATEMENT (pas juste à la fin)

Comme ça : on atteint 66% à epoch 25, sauvegardé direct sur disque, peu importe
si NaN cascade après. C'est essentiellement run 42 mais robuste.
"""
...
# ============================================================
EXPERIMENT_NAME = "lift/44_lift_bn_trainable_stable"
...
class ImageStateE2E(nn.Module):
    ...
    def __init__(self, state_dim, hidden_dims, chunk_size, action_dim, n_bins,
        ...
        # ResNet18 (pré-entraîné ImageNet, BN trainable — comme run 42)
        # Les BN apprennent les stats Lift, c'est ce qui débloque les 66%
