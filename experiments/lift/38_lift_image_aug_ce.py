"""
Expérience 38 — Lift : image augmentation + CE+résidu (vs run 36 baseline).

Réplique exacte du run 36 (image features + state + CE+résidu = best 34%),
SAUF qu'au training on utilise des features augmentées (8 versions par frame
avec RandomResizedCrop + ColorJitter) sélectionnées au hasard à chaque sample.

But : tester si l'image augmentation réduit l'overfit (test loss qui montait
de 0.97 à 1.52 en run 36) et améliore le best success rate.

À l'éval : on rend live, pas d'augmentation (cohérent avec la déployment).
"""
...
# ============================================================
EXPERIMENT_NAME = "lift/38_lift_image_aug_ce"
...
def train_model(model, XF, XS, Y, train_idx, test_idx, n_versions):
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0, len(train_idx), BATCH_SIZE):
            ...
            # Sélection aléatoire d'une version augmentée par sample du batch
            ver_ids = torch.randint(0, n_versions, (len(batch),), generator=gen)
...
def main():
    label = "mlp_feat_state_ce_aug"
    arch_name = f"ResNet18(gelé,aug) + MLP {FEATURE_DIM+STATE_DIM} → {' → '.join(str(h) for h in HIDDEN_DIMS)} → CE+résidu (n_aug={N_VERSIONS})"
    ...
    model_config = {
        ...
        "note": "Run 36 + image augmentation (RandomResizedCrop + ColorJitter, 8 versions/frame).",
    }
    ...
    run_data = build_run_data(
        ...
        extra={
            ...
            "description": "Run 36 + image augmentation 8x pour fight overfit.",
        },
    )
    save_run(run_data)
