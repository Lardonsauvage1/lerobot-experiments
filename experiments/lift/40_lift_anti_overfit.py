"""
Expérience 40 — Lift : combo anti-overfit sur la baseline run 36 (best 34%).

Tous les runs Lift précédents (33-39) souffraient d'un overfit massif :
train loss baissait, test loss explosait, success rate s'effondrait
passé l'epoch 25.

Combo de 4 régularisations classiques en parallèle :
- Dropout p=0.4 entre couches MLP
- LayerNorm après chaque couche cachée (comme Robomimic)
- AdamW avec weight_decay=5e-4 (au lieu d'Adam vanilla)
- Label smoothing 0.1 sur la CE (empêche over-confidence)

Tout le reste identique au run 36 (l'archi de base à 34%).
Plus : éval finale à 200 épisodes au lieu de 50, pour réduire variance
de mesure (±1.7pt vs ±6.5pt) et avoir un chiffre comparable propre.
"""
...
# ============================================================
EXPERIMENT_NAME = "lift/40_lift_anti_overfit"
...
def main():
    ...
    arch_name = (f"ResNet18(gelé) + MLP {FEATURE_DIM+STATE_DIM} → "
