"""
Expérience 36 — Lift : features image ResNet + state + CE-discret par dim.

Combine les 2 enseignements qui ont marché :
- Run 35 PushT-like : features ResNet18 (gelé) + state low-dim → +28pts vs state-seul
- Run 31 PushT      : CE sur bins discrets + résidu → bat MSE sur multimodal

Plan : même backbone que run 35, mais sortie CE par dim d'action (run 34 style)
au lieu de MSE continue. Best test des deux mondes.
"""
...
# ============================================================
EXPERIMENT_NAME = "lift/36_lift_image_state_ce"
...
class MLPImageStateDiscrete(nn.Module):
    """MLP qui prend (features ResNet + state) → bins + résidus par dim d'action."""
...
def main():
    label = "mlp_feat_state_ce"
    arch_name = f"ResNet18(gelé) + MLP {FEATURE_DIM+STATE_DIM} → {' → '.join(str(h) for h in HIDDEN_DIMS)} → CE+résidu (N={N_BINS}×{ACTION_DIM}×{CHUNK_SIZE})"
    ...
    model_config = {
        ...
        "note": "Image features + state → CE+résidu (run 31 PushT formula sur Lift).",
    }
    ...
    run_data = build_run_data(
        ...
        extra={
            ...
            "description": "Lift : features ResNet + state → CE+résidu (run 31 PushT formula appliquée à Lift).",
        },
    )
    save_run(run_data)
