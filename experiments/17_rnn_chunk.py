"""
Expérience 17 — CNN + RNN (GRU) + Action Chunking.

Compare avec le Transformer (51% coverage sur 50 épisodes).
Mêmes conditions exactes.
"""
...
class RNNChunkPolicy(nn.Module):
    """CNN + GRU + action chunking.

    Architecture :
        Image → CNN → features (64D)
        features + position (2D) → GRU → hidden state
        hidden state → Linear → chunk de 20 actions (40 valeurs)

    Le GRU traite l'observation comme une séquence de 1 élément.
    L'état caché persiste entre les appels pendant l'évaluation,
    donnant au modèle une mémoire des observations précédentes.
    """

    def __init__(self, cnn_channels, cnn_feature_dim, image_size,
                 rnn_hidden, rnn_layers, chunk_size, pos_dim=2, action_dim=2):
        super().__init__()
    ...
    def forward(self, image, agent_pos, hidden=None):
        """
        Entraînement : image (batch, 3, H, W), agent_pos (batch, 2)
        Retourne : actions (batch, chunk_size, action_dim), hidden
        """
        feats = self.cnn(image)
        x = torch.cat([feats, agent_pos], dim=1).unsqueeze(1)  # (batch, 1, input_dim)
        rnn_out, hidden = self.rnn(x, hidden)
        out = self.fc_out(rnn_out[:, -1, :])  # (batch, chunk_size * action_dim)
        actions = out.reshape(-1, self.chunk_size, self.action_dim)
...
def train_model(model, X_img, X_pos, Y):
    ...
    run_dir = get_run_dir(EXPERIMENT_NAME, "gru_chunk20")
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0, len(train_idx), BATCH_SIZE):
            ...
            pred, _ = model(X_img[batch], X_pos[batch])
        ...
        model.eval()
        with torch.no_grad():
            test_pred, _ = model(X_img[test_idx], X_pos[test_idx])
        ...
        # Checkpoint toutes les 100 epochs
        if (epoch + 1) % 100 == 0:
            import shutil
    ...
    for ep in range(N_EVAL_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if len(action_buffer) == 0:
                ...
                with torch.no_grad():
                    chunk_pred, hidden = model(img, agent_pos, hidden)
...
def main():
    label = "gru_chunk20"
    arch_name = f"CNN+GRU(h={RNN_HIDDEN},L={RNN_LAYERS}) chunk={CHUNK_SIZE}"
    ...
    # 5. Inférence
    inf_time = measure_inference_time(model, {"image": dummy_img, "agent_pos": dummy_pos})
    print(f"  Inférence : {inf_time:.2f}ms/appel ({inf_time/CHUNK_SIZE:.2f}ms/frame)")

    # 6. Évaluation 50 épisodes
    ...
    run_data = build_run_data(
        ...
        extra={
            ...
            "chunk_size": CHUNK_SIZE,
            "description": f"CNN + GRU (hidden={RNN_HIDDEN}, {RNN_LAYERS} couches) + action chunking={CHUNK_SIZE}",
        },
    )
    save_run(run_data)
    ...
    print(f"\n{'='*60}")
    print(f"RÉSUMÉ — RNN + chunk={CHUNK_SIZE}")
    print(f"{'='*60}")
