"""
Expérience 29 — Image seule (sans agent_pos) → chunk 20 actions.

Variante minimale de 24_precomputed_features : on retire agent_pos de
l'entrée du Transformer. Objectif : voir si la loss devient mieux corrélée
au coverage quand le modèle ne peut plus prendre le raccourci
"renvoie current_pos + petit delta".

Tout le reste est identique au run 24 (best run actuel à 46% coverage) :
mêmes features ResNet18 pré-calculées, même Transformer 2L, mêmes
hyperparams, même seed.
"""
...
N_EVAL_EPISODES = 200  # 200 eps pour réduire la variance (±1.7pt vs ±3.7pt à 50)
MAX_STEPS = 300
...
def extract_features():
    ...
    features = []
    batch_size = 128
    print(f"  {len(X_img)} images à traiter...")
    with torch.no_grad():
...
class TransformerChunkImageOnly(nn.Module):
    """Transformer qui prend UNIQUEMENT les features image (pas de agent_pos).

    Entrée : features (512D) → projeté en d_model
    Sortie : chunk de 20 actions
    """

    def __init__(self, feature_dim, d_model, n_heads, n_layers, dim_feedforward,
                 chunk_size, action_dim=2):
        super().__init__()
...
def load_data_chunked(features, actions, episodes):
    """Chunk les features pré-calculées (positions ignorées)."""
...
def train_model(model, X_feat, Y):
    ...
    for epoch in range(EPOCHS):
        ...
        model.eval()
        with torch.no_grad():
            test_pred = model(X_feat[test_idx])
...
def eval_in_simulation(model, norm):
    """Évalue en simulation — image seule en entrée."""
    ...
    for ep in range(N_EVAL_EPISODES):
        ...
        if (ep + 1) % 20 == 0:
            ...
            print(f"    Episode {ep+1:3d}/{N_EVAL_EPISODES} : {status} coverage={max_coverage:.1%}")
        if frames:
...
def main():
    label = "resnet18_image_only"
    arch_name = f"ResNet18(gelé) → Transformer 2L (image seule, sans agent_pos) chunk={CHUNK_SIZE}"
    ...
    model = TransformerChunkImageOnly(
    ...
    print(f"  Paramètres entraînés : {info['total_params']:,}")
    print(f"  (ResNet18 gelé : 11.2M params, non entraînés)")

    print("\nEntraînement (image seule, sans agent_pos)...")
    train_losses, test_losses, epoch_times, train_time, n_train, n_test = train_model(model, X_feat, Y)
    ...
    print(f"  Inférence Transformer : {inf_time:.2f}ms")
    print(f"  (+ ResNet18 ~5ms en inférence live)")

    eval_results, all_frames = eval_in_simulation(model, norm)

    run_dir = get_run_dir(EXPERIMENT_NAME, label)
    model_config = {
        "class": "TransformerChunkImageOnly",
        "feature_dim": FEATURE_DIM, "d_model": D_MODEL, "n_heads": N_HEADS,
        "n_layers": N_LAYERS, "dim_feedforward": DIM_FEEDFORWARD,
        "chunk_size": CHUNK_SIZE,
        "note": "Image seule en entrée (pas de agent_pos) — comparable au run 24",
    }
    ...
    run_data = build_run_data(
        ...
        flops_inference=flops_transformer,
        ...
        extra={
            ...
            "frozen_params": 11_176_512,
            "description": "Image seule (sans agent_pos) → Transformer pour action chunking. Comparaison au run 24 pour mesurer l'apport de agent_pos.",
        },
    )
    save_run(run_data)
    ...
    print(f"  Params entraînés : {info['total_params']:,}")
    print(f"  Temps train      : {train_time:.0f}s")
    print(f"  Inférence        : {inf_time:.2f}ms (Transformer seul)")
