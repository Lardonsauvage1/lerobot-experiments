"""
Expérience 28 — Baseline propre sans historique de positions.

Même archi que 27 (MLP [256,128] chunk=20, SEED=42, sans dropout/wd)
mais sans historique — juste features ResNet + position courante.

10 000 epochs, éval coverage tous les 200 epochs (50 points).
Le point epoch=1000 est directement comparable à 27_hist5.
"""
...
# ============================================================
EXPERIMENT_NAME = "28_baseline_no_hist"
...
class MLPChunk(nn.Module):
    """MLP sans dropout : features ResNet + position courante -> chunk d'actions."""
...
def eval_with_videos(model, norm):
    """Évaluation finale complète avec vidéos."""
    import gymnasium as gym
    import gym_pusht  # noqa
...
def main():
    label = f"ep{EPOCHS}"
    arch_name = f"ResNet18 → MLP {HIDDEN_LAYERS} chunk={CHUNK_SIZE} no-hist"
    ...
    print(f"  Dropout    : 0  (baseline pur)")
    print(f"  Weight dec : 0")
    ...
    for epoch in range(EPOCHS):
        ...
        # Éval coverage tous les EVAL_EVERY epochs
    ...
    run_data = build_run_data(
        ...
        extra={
            ...
            "description": f"Baseline sans history, {EPOCHS} epochs, eval tous les {EVAL_EVERY}",
        },
    )
    save_run(run_data)
    ...
    if coverages_cov:
        print(f"  Meilleur cov.   : {max(coverages_cov):.1f}% @ epoch {epochs_cov[best_idx]}")
