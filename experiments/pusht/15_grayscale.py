"""
Expérience 15 — Images en noir et blanc + CNN réduit.

On convertit les images en niveaux de gris (1 canal au lieu de 3)
et on teste avec des CNN de tailles différentes.

Runs :
  1. NB + CNN [16, 32] (même taille de CNN)
  2. NB + CNN [8, 16] (réduit)
  3. NB + CNN [4, 8] (mini)
"""
...
class SimpleCNN(nn.Module):
    def __init__(self, channels, feature_dim, image_size, in_channels=1):
        super().__init__()
        conv_layers = []
        in_ch = in_channels
...
class TransformerChunkPolicy(nn.Module):
    def __init__(self, cnn_channels, cnn_feature_dim, image_size,
                 ...
                 chunk_size, in_channels=1, pos_dim=2, action_dim=2):
        super().__init__()
        self.cnn = SimpleCNN(cnn_channels, cnn_feature_dim, image_size, in_channels=in_channels)
...
def load_data_grayscale():
    print(f"Chargement (chunk={CHUNK_SIZE}, grayscale)...")
    X_img, X_pos, Y_single, norm, meta = load_pusht_cached(
        n_episodes=N_EPISODES_TRAIN, image_size=IMAGE_SIZE, seq_len=1
    )

    # Convertir en noir et blanc : moyenne des 3 canaux
    # (3, 64, 64) → (1, 64, 64)
    X_img_gray = X_img.mean(dim=1, keepdim=True)
    print(f"  Images couleur : {X_img.shape} → NB : {X_img_gray.shape}")
    ...
    i = 0
    while i + CHUNK_SIZE <= len(X_img_gray):
        if all_eps[i] == all_eps[i + CHUNK_SIZE - 1]:
            valid_imgs.append(X_img_gray[i])
...
def main():
    cnn_str = str(CNN_CHANNELS).replace(" ", "")
    label = f"gray_cnn{cnn_str}"
    arch_name = f"CNN{cnn_str} gray + Transformer {N_LAYERS}L chunk={CHUNK_SIZE}"
    ...
    # 1. Charger
    X_img, X_pos, Y, norm = load_data_grayscale()
    ...
    model = TransformerChunkPolicy(
        ...
        chunk_size=CHUNK_SIZE, in_channels=1,
    )
    ...
    flops_inf = count_flops(model, {
        "image": torch.randn(1, 1, IMAGE_SIZE, IMAGE_SIZE),
    ...
    run_data = build_run_data(
        ...
        extra={
            "cnn_channels": CNN_CHANNELS, "grayscale": True,
            "description": f"Images NB + CNN {CNN_CHANNELS} + Transformer chunk={CHUNK_SIZE}",
        },
    )
    save_run(run_data)
    ...
    print(f"\n{'='*60}")
    print(f"RÉSUMÉ — NB + CNN {CNN_CHANNELS}")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cnn", nargs="+", type=int, default=[16, 32])
