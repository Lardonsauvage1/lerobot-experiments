...
echo "[apple-b2-perf-$RES $(date '+%H:%M')] 1cam fixe ${RES}px 10Hz, R34+$DIMS, DDIM, horizon16/nact8, const+EMA, $STEPS steps"
EMA=1 CONST_LR=1 CONST_LR_VALUE=1e-4 python -u experiments/lift/50_train_valloss.py \
  ...
  --output_dir=results/runs/real/apple_b2_perf_$RES \
  --batch_size=64 --steps=$STEPS --save_freq=2000 --log_freq=50 --eval_freq=0 \
  --seed=42 --wandb.enable=false --num_workers=8 2>&1 | tee /tmp/apple_b2_perf_$RES.log
echo "[apple-b2-perf-$RES $(date '+%H:%M')] TRAIN DONE"
