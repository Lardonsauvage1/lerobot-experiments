#!/usr/bin/env bash
# Cooldown apple_b2_fixed_$RES (LR 1e-4->0/5k, EMA). Arg = résolution.
set -u
RES=$1
...
echo "[cd-b2-$RES $(date '+%H:%M')] cooldown 5k -> $RUN/cooldown"
EMA=1 COOLDOWN_STEPS=5000 COOLDOWN_LR0=1e-4 python -u experiments/lift/50_train_valloss.py \
  ...
  --output_dir=$RUN/cooldown --steps=5000 --save_freq=1000 --log_freq=50 --eval_freq=0 2>&1 | tee /tmp/cd_apple_b2_$RES.log
echo "[cd-b2-$RES $(date '+%H:%M')] COOLDOWN DONE"
