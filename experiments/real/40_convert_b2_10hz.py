...
Sorties : local/apple_b2_10hz_fixed_96 et local/apple_b2_10hz_fixed_128."""
import shutil
...
def main():
    ...
    print(f"[b2-10hz] {len(eps)} ép, {nj} joints, résolutions {RES}")
    dsets = {}
    for r in RES:
        root = Path(f"data_cache/lerobot_apple_b2_10hz_fixed_{r}")
        ...
        dsets[r] = LeRobotDataset.create(repo_id=f"local/apple_b2_10hz_fixed_{r}", fps=int(FS), features=feats, root=root,
    ...
    for bag in eps:
        try:
            ...
            print(f"[b2-10hz] !! {bag.name} ECHEC: {e} -> skip"); continue
        for fr in frames:
            for r in RES:
        ...
        total += len(frames)
        print(f"[b2-10hz] {bag.name}: {len(frames)} frames (cumul {total})", flush=True)
    for r in RES:
        print(f"[b2-10hz] TERMINE {r}px: {dsets[r].meta.total_episodes} ép, {total} frames")
