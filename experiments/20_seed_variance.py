"""
Expérience 20 — Variabilité selon la seed.

On entraîne 3 fois le même modèle (Transformer 2L, chunk=20, 206 épisodes, 300 epochs)
avec des seeds différentes pour mesurer la variabilité des résultats.
"""
...
# ============================================================
EXPERIMENT_NAME = "20_seed_variance"
...
def run_with_seed(seed):
    ...
    SEED = seed
    label = f"seed{seed}"
    arch_name = f"Transformer 2L chunk={CHUNK_SIZE}"
    ...
    run_data = build_run_data(
        ...
        extra={"seed": seed, "description": f"Transformer 2L chunk=20, seed={seed}"},
    )
    save_run(run_data)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"COMPARAISON DES SEEDS")
    print(f"{'='*60}")
    for r in all_results:
        print(f"  Seed {r['seed']:4d} : coverage={r['avg_coverage']:.1%}, success={r['success_rate']:.0%}")
